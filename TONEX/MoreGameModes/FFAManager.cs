            ...
            if (AmongUsClient.Instance.AmHost)
            {
                ...
                if (FFA_EnableRandomTwists.GetBool() && FFAdoTP)
                {
                    ...
                    foreach (PlayerControl pc in Main.AllAlivePlayerControls)
                    {
                        ...
                        var pcList = Main.AllAlivePlayerControls.Where(x => x.PlayerId != pc.PlayerId && x.IsAlive()).ToList();
                        PlayerControl target = pcList[IRandom.Instance.Next(0, pcList.Count)];
