...
public abstract class AddonBase : IDisposable
{
    #region 玩家相关处理
    /// <summary>
    /// 玩家本体
    ...
    )
    {
        ...
        var adb = new List<AddonBase>();
        if (!CustomRoleManager.AllActiveAddons.ContainsKey(Player.PlayerId))
            CustomRoleManager.AllActiveAddons.Add(Player.PlayerId,  adb);
...
#pragma warning restore CA1816
    ...
    #region RPC相关处理
    /// <summary>
    /// AddonBase 专用 RPC 发送类
    /// 会自动发送 PlayerId
    /// </summary>
    public class RoleRPCSender : IDisposable
    {
        public MessageWriter Writer;
        public RoleRPCSender(AddonBase role, CustomRPC rpcTypes)
        {
            Writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId, (byte)rpcTypes, SendOption.Reliable, -1);
    ...
    protected RoleRPCSender CreateSender(CustomRPC rpcTypes)
    {
        return new RoleRPCSender(this,  rpcTypes);
    }
    /// <summary>
    /// 接受到 RPC 时的函数
    ...
    public virtual void ReceiveRPC(MessageReader reader, CustomRPC rpcTypes)
    { }
    #endregion
    #region 基层设定相关处理
    /// <summary>
