...
public sealed class Cupid : RoleBase, INeutralKiller
{
    ...
    public static void SetupOptionItem()
    {
        ...
        CupidLovers.CupidLoverKnowRoles = BooleanOptionItem.Create(RoleInfo, 12, OptionName.LoverKnowRoles, true, false);
        CupidLoverKnowCupid = BooleanOptionItem.Create(RoleInfo, 14, OptionName.CupidLoverKnowCupid, true, false);
        CupidLovers.CupidLoverSuicide = BooleanOptionItem.Create(RoleInfo, 13, OptionName.LoverSuicide, true, false);
    }
    ...
    public bool OnCheckMurderAsKiller(MurderInfo info)
    {
        ...
        if (CanBeLover(target))
        {
            ...
            if (ReadyPlayers.Count == 2 && CupidLimit == 0)
            {
                foreach (var id in ReadyPlayers)
                {
                    CupidLovers.CupidLoversPlayers.Clear();
                    CupidLovers.isCupidLoversDead = false;
                    CupidLovers.CupidLoversPlayers.Add(Utils.GetPlayerById(id));
        ...
        if (target.Is(CustomRoles.CupidLovers))
        {
            foreach(var pc in Main.AllPlayerControls.Where(p=>p.Is(CustomRoles.Cupid)))
            {
                ...
                if (roleclass.Shield)
                {
                    ...
                    killer.SetKillCooldownV2();
                    foreach (var p in CupidLovers.CupidLoversPlayers)
